...
def test_force_readonly_enforcement():
    """
    ...
    """
    # Create mock for connection URL
    mock_conn_url = "postgresql://user:password@localhost/test"
    ...
    with patch("pg_mcp.server.current_access_mode", AccessMode.UNRESTRICTED), patch(
        "pg_mcp.server.connection_registry.get_connection", return_value=mock_conn_url
    ), patch.object(SqlDriver, "_execute_with_connection", mock_execute), patch("pg_mcp.sql_driver.psycopg.connect"):
        ...
        # Test explicit True
        mock_execute.reset_mock()
        driver.execute_query("SELECT 1", force_readonly=True)
        assert mock_execute.call_count == 1
        ...
        # Test explicit False
        mock_execute.reset_mock()
        driver.execute_query("SELECT 1", force_readonly=False)
        assert mock_execute.call_count == 1
        # Check that force_readonly=False is respected
        assert mock_execute.call_args[1]["force_readonly"] is False

    # Test RESTRICTED mode
    with patch("pg_mcp.server.current_access_mode", AccessMode.RESTRICTED), patch(
        "pg_mcp.server.connection_registry.get_connection", return_value=mock_conn_url
    ), patch.object(SqlDriver, "_execute_with_connection", mock_execute), patch("pg_mcp.sql_driver.psycopg.connect"):
        driver = get_sql_driver(conn_name="default")
