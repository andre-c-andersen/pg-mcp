...
def mock_sql_driver():
    ...
    driver.execute_query = Mock(return_value=[])
    return driver
...
def test_multiple_queries(safe_driver, mock_sql_driver):
    ...
    safe_driver.execute_query(query1)
    safe_driver.execute_query(query2)
    mock_sql_driver.execute_query.assert_has_calls(
        [
            call("/* pg-mcp */ " + query1, params=None, force_readonly=True),
            call("/* pg-mcp */ " + query2, params=None, force_readonly=True),
        ]
    )
